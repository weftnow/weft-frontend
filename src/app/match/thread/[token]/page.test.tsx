...
test("a thread nobody has answered says so and does not apologise", () => {
  ...
  expect(html).toContain(demoB2cContent.thread.waiting.headline);
  // The list heading belongs to MatchesView, a different screen entirely.
  expect(html).not.toContain(escapeApostrophes(demoB2cContent.matches.headline));
});
...
test("an unknown token says the link is not recognised", () => {
  ...
  expect(html).toContain(escapeApostrophes(demoB2cContent.thread.unknown.headline));
});
...
test("an outage offers a retry rather than a dead end", () => {
  ...
  expect(html).toContain(escapeApostrophes(demoB2cContent.thread.unavailable.headline));
});
